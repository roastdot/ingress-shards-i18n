...
export function processSeriesData(seriesDataPackage: any): Record<string, any> {
    ...
    const allObservedOrnamentedPortals = ornamentedPortals?.flatMap((exportObj: any) => {
        const observedAt = exportObj.timestamp ? Instant.epochMilliseconds(Instant.fromString(exportObj.timestamp)) : 0;
        return exportObj.portals.map((p: any) => ({ ...p, observedAt }));
    }) || [];

    const allObservedTargetPortalsByFaction: Record<string, any[]> = {
        RES: [],
        ENL: [],
    };

    targetPortals?.forEach((exportObj: any) => {
        exportObj.artifact.forEach((artifact: any) => {
            const abbreviatedTeam = artifact.id === 'targetres' ? 'RES' : artifact.id === 'targetenl' ? 'ENL' : null;
            if (!abbreviatedTeam) return;

            artifact.target.forEach((t: any) => {
        ...
        for (const o of allObservedOrnamentedPortals) {
            ...
            if (matches.length === 1) {
                ...
                if (!loggedAmbiguous.has(portalKey)) {
                    console.error(`❌ Ornament "${o.title}" at ${o.lat}, ${o.lng} matches multiple sites: ${matches.map((m: any) => m.name).join(', ')}. Skipping.`);
    ...
    const targetsBySite = new Map<string, any[]>();
    ...
    sitesGeocode.forEach((siteGeocode: any) => {
    ...
    const siteFragmentsMap = new Map<string, any[]>();
    for (const sjt of shardJumpTimes) {
        const artifacts = sjt.artifact.filter((d: any) => d.fragment);
        ...
        for (const artifact of artifacts) {
            ...
            const addFragmentToSite = (fragment: any, siteId: string) => {
                ...
                */
                const spawnEvents = fragment.history.filter((h: any) => h.reason === HISTORY_REASONS.SPAWN).length;
                if (spawnEvents > 1) {
                    const fragments: any[] = [];
                    let splitFragment: any;
                    for (const historyItem of fragment.history.sort((a: any, b: any) => a.moveTimeMs.localeCompare(b.moveTimeMs))) {
...
export function processSite(site: any, siteOrnamentedPortals: any, siteTargetPortals: any, fragments: any, seriesConfig: any, blueprints: any, verbose = false): any {
...
function applyTargetPortalsToSite(site: any, siteTargetPortals: any): void {
    siteTargetPortals.forEach((t: any) => {
...
function applyFragmentsToSite(site: any, fragments: any[], siteTargetPortals: any[], seriesConfig: any, blueprints: any, verbose: boolean): void {
    const siteEventType = site.geocode.eventType;
    const shardComponents = seriesConfig.shardComponents || [];
    const seriesEventConfig = shardComponents.find((et: any) => et.eventType === siteEventType);

    // Find site-specific config for overrides
    let siteConfig: any = null;
    seriesEventConfig?.schedule?.forEach((sched: any) => {
        const found = sched.sites?.find((s: any) => s.name === site.geocode.name);
    ...
    if (shardMechanic && shardMechanic.waves && shardMechanic.waves.length > 1) {
        ...
        shardMechanic.waves.forEach((wave: any, index: number) => {
            ...
            const waveFragments = fragments.filter(fragment => {
                const spawnTime = getFragmentSpawnTimeMs(fragment) ?? 0;
            ...
            const waveViewData = processFragments({
                ...
                portalLookup: site[portalLookupByOriginalKey],
                sitePortals: site.portals,
                siteTargetPortals: siteTargetPortals.filter((t: any) => t.observedAt >= waveStart.getTime() && t.observedAt < waveEnd.getTime()),
                eventType: siteEventType,
                geocode: site.geocode,
                fullEvent: false,
...
function processFragments({ fragments, portalLookup, sitePortals, siteTargetPortals, eventType, geocode, fullEvent, verbose = false }: any): any {
    const siteName = geocode.name;
    // `any` deliberately: this object's shape is mutated dynamically below
    // (see the pragmatic-typing note for this file).
    const viewData: any = {
        shards: [],
        shardPaths: {},
        targets: {
    ...
    if (siteTargetPortals) {
        ...
        siteTargetPortals.forEach((t: any) => {
            const portalKey: any = getPortalKey({ title: t.title, latE6: Number(t.latE6), lngE6: Number(t.lngE6) });
        ...
        viewData.targets.RES = [...new Set<any>(viewData.targets.RES)].sort((a, b) => a - b);
        viewData.targets.ENL = [...new Set<any>(viewData.targets.ENL)].sort((a, b) => a - b);

        viewData.counters.targets.RES = viewData.targets.RES.length;
        viewData.counters.targets.ENL = viewData.targets.ENL.length;
    }

    for (const fragment of fragments.sort((a: any, b: any) => a.id.localeCompare(b.id))) {
        const shardId = parseInt(fragment.id.includes('_') ? fragment.id.slice(fragment.id.lastIndexOf('_') + 1) : fragment.id, 10);

        let mostRecentShardPortalKey: any;
        let shard: any;
        let allowFurtherPoints = true;
        for (const historyItem of fragment.history.sort((a: any, b: any) => a.moveTimeMs.localeCompare(b.moveTimeMs))) {
            let originPortalKey: any = getPortalKey(historyItem.originPortalInfo);
            const destPortalKey: any = getPortalKey(historyItem.destinationPortalInfo);
            ...
            switch (historyItem.reason) {
                ...
                case HISTORY_REASONS.LINK: {
                    ...
                    const existingPath = viewData.shardPaths[pathKey];
                    if (existingPath) {
                        const existingLink = existingPath.links.find((link: any) => link.linkTime === linkTime);
                        if (existingLink) {
...
function findSiteForFragment(fragment: any, sitesGeocode: any[]): any {
    ...
    let matchedSite = sitesGeocode.find(site => {
        ...
        const matchingDate = isWithin24Hours(getFragmentSpawnTimeMs(fragment) ?? 0, site.dateMillis);
...
function applyFragmentPortalsToSite(site: any, fragments: any[]): void {
...
function applyOrnamentedPortalsToSite(site: any, siteOrnamentedPortals: any[]): void {
    for (const p of siteOrnamentedPortals) {
        const key: any = getPortalKey(p);
...
function createPortalForSite(site: any, originalPortalKey: any, portalInfo: any): any {
    let newPortal = null;
...
function getLinkRule(rules: any, distance: number): any {
