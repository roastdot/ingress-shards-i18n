...
export function validateProcessedSeriesData(processedSeriesData: Record<string, any>, seriesConfig: any, blueprints: any, verbose = false): any {
...
function validateSites(processedSites: any[], seriesConfig: any, blueprints: any, verbose = false): any {
    const seriesMissing: any[] = [];
    const seriesOutsideWindow: any[] = [];
    const seriesInvalidSequences: any[] = [];
    const seriesMismatches: any[] = [];

    const seriesValidation: { eventTypeConfigs: Record<string, any> } = {
        eventTypeConfigs: {},
    };

    if (seriesConfig.shardComponents) {
        seriesConfig.shardComponents.forEach((componentConfig: any) => {
            ...
            const totalShards = shardMechanic?.waves.reduce((sum: number, wave: any) => sum + (wave.quantity || 0), 0) || 0;

            const totalTargets =
                targetMechanic?.waves.reduce((sum: number, wave: any) => {
                    const factions = wave.factionQuantity || {};
                    const waveTotal = Object.values(factions).reduce((a: any, b: any) => a + b, 0);
                    return sum + (waveTotal as number);
                }, 0) || 0;
    ...
    for (const site of processedSites) {
        if (Object.entries(seriesValidation.eventTypeConfigs).length > 0) {
            ...
            componentConfig?.schedule?.forEach((sched: any) => {
            ...
            if (eventTypeConfig) {
                ...
                const expectedShards = (siteConfig && siteConfig.shardCounts)
                    ? siteConfig.shardCounts.reduce((sum: number, count: number) => sum + count, 0)
                    : totalShards;
                ...
                if (site.waves && shardMechanic.waves) {
                    ...
                    const missingShardActions: any[] = [];
                    const shardActionsOutsideJumpWindow: any[] = [];
                    const invalidShardSequences: any[] = [];
                    let invalidDespawnsCount = 0;
                    for (const [waveIndex, wave] of site.waves.entries() as IterableIterator<[number, any]>) {
                        ...
                        for (const shard of wave.shards) {
                            ...
                            if (shard.history.length !== expectedWaveSchedule.length) {
                                for (const historyItem of shard.history) {
                                    ...
                                    const expectedIndex = expectedWaveSchedule.findIndex((wsa: any) => {
                                        const actionMatches = (
                            ...
                            let currentLocationId: any = null;
        ...
        if (site.fullEvent) {
            for (const [shardPathKey, shardPath] of Object.entries(site.fullEvent.shardPaths) as Array<[string, any]>) {
        ...
        if (site.fullEvent?.alignmentMismatches?.length > 0) {
            const tableData = site.fullEvent.alignmentMismatches.map((mismatch: any) => {
                const waveIndex = site.waves ? site.waves.findIndex((w: any) => w.shards.some((s: any) => s.id === mismatch.shardId)) : -1;
