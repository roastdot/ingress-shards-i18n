 ...
 */
function getSpawnHistoryItemForFragment(fragment: any): any {
    return fragment.history
        .sort((a: any, b: any) => a.moveTimeMs.localeCompare(b.moveTimeMs))
        .find((h: any) => h.reason === HISTORY_REASONS.SPAWN);
}

export function getCoordsForFragment(fragment: any): { latitude: number; longitude: number } {
...
export function getFragmentSpawnTimeMs(fragment: any): number | null {
    const spawnHistoryItem = getSpawnHistoryItemForFragment(fragment);
    return spawnHistoryItem ? Number(spawnHistoryItem.moveTimeMs) : null;
}

export function calculateCentroid(portalsMap: Record<string, any> | null | undefined): { lat: number; lng: number } | null {
    ...
    for (const id of portalIds) {
        const portal = (portalsMap as Record<string, any>)[id];
 ...
 */
export function getPortalKey(portal: any): string | null {
...
export function printTable(data: Array<Record<string, any>> | null | undefined): void {
...
export function calculateShardActionSchedule(shardMechanic: any, siteGeocode: any): any {
    ...
    for (const wave of shardMechanic.waves) {
        ...
        const waveSchedule: any[] = [];
        for (const waveAction of shardMechanic.waveActions.filter((a: any) =>
...
export function convertToCsv(data: Array<Record<string, any>> | null | undefined): string {
    if (!data || data.length === 0) return '';
...
export function formatZonedDateTimeWithMs(zdt: any): string {
    if (!zdt) return '-';
...
export function formatTimeWithMs(epochMs: number | string | null | undefined, timezone: string): string {
    if (!epochMs) return '-';
...
export function formatDurationMs(durationMs: number, showDecimals = true): string {
